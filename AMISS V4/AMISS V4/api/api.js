...
var urli = "http://amiss4.ifc.com.vn:1005/ServiceChung"
var urlidtt = "https://tcpip.amiss.com.vn/ServiceTCPAll"; // cài trên server 75

//Sql1Pha3Pha
//getMenuUser();

// gui tcp - doc tuc thoi
...
function api_ExecuteTcpString(config, para) {
    try {
        ...
        $.ajax({
            url: urlidtt + "/api/TCP_OverServiceTcpString",
            type: "POST",
            data: jsonpara,
            async: syns,
...
function ExecuteServiceSyns(config, para) {
    try {
        ...
        $.ajax({
            ...
            data: jsonpara,
            async: syns,
            //headers: {'x-my-custom-header': 'send_tokenkey_o_day'},
            success: function (data, textStatus, jqXHR) {
...
// Format toString kiểu dữ liệu date
// dùng được cho tất cả các loại format
...
function f_importExcelToOracle(config, fd) {
    try {
        var lstData = null;
        $.ajax({
            type: 'POST',
            url: urli + '/InportFileExcelToOracle/InportExcelToOrcale',
            data: fd,
            processData: false,
            contentType: false,
            dataType: "json",
...
function f_importFileXml2(config, fd) {
    try {
        var lstData = null;
        $.ajax({
            type: 'POST',
...
function ExecuteBulkCopyOracle(config, table) {
    try {
        console.log("ExecuteBulkCopyOracle");
...
function LoadPhanTrang_(idPage, idCurentPage, data, func) {
    try {
        ...
        var html = '<span class="p_prev" valuePage ="1">← Đầu</span>' +
                        '<span class="p_active" valuePage ="' + (tranghientai == 1 ? 1 : (tranghientai - 1)) + '">←</span>' +
                        '<input type="number" value="' + tranghientai + '" class="" id="pagenumber" size="1" valuePage="' + tranghientai + '"/>' +
                        '<span class="p_active" valuePage="' + (tranghientai == tongtrang ? tongtrang : (tranghientai + 1)) + '">→</span>' +
                        '<span class="p_next" valuePage="' + tongtrang + '">Cuối → </span>';
...
function ExecuteExportXmlOracle(config, para, colum) {
    try {
        var idrandom = Math.random().toString(36).substr(2);
        ...
        $.ajax({
            url: urli + "/api/Xml_ExportOracle",
            type: "POST",
            data: jsonpara,
            async: true,
            success: function (data, textStatus, jqXHR) {
                window.location = urli + "/api/Xml_ExportOracle?idrandom=" + idrandom;
            },
            complete: function () {
            },
            error: function (jqXHR, textStatus, errorThrown) {
...
function api_sendMail(config) {
    var jsonpara = { config: JSON.stringify(config) };
    $.ajax({
        url: urli + "/api/Email_SendMail",
        type: "POST",
        data: jsonpara,
        async: true,
        success: function (data, textStatus, jqXHR) {
            var lstData = data == null || data == undefined || data == "" ? "[]" : JSON.parse(data);
            if (config.callback != null && config.callback != '') {
                funExeItem(config.callback, config, [], lstData);
            }
        },
...
function genTable(id, temp, js) {
    var rowinpage = [10, 20, 50];
    $("#sobanghi_1trang").empty();
    var opt = "";
    $.each(rowinpage, function (key, val) {
        if (val < js.length)
            opt += '<option value="' + val + '">' + val + '</option>';
    })
    if (rowinpage[2] < js.length) {
        opt += '<option value="' + js.length + '">'+js.length+'</option>';
    }

    $("#sobanghi_1trang").append(opt);
    if (js.length < 10)
        $("#sobanghi_1trang").hide();
    else
        $("#sobanghi_1trang").show();

    $("#sobanghi_1trang").change(function () {
        $('#' + id).renderTable({
            template: temp,
            data: js,
            pagination: {
                rowPageCount: $("#sobanghi_1trang option:selected").val()
            },
        });
    })
    $('#' + id).renderTable({
