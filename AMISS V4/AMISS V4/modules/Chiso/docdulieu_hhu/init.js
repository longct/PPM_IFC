    ...
    try {
        ...
        $("#btnDoc").click(function () {
            try {
                ...
                if (distance > 30) {
                    alert("Hãy chọn trong khoảng 30 ngày gần nhất", "error");
                    return;
                }
            } catch (e) {
...
function statusProsessBar() {
    try {
        ...
        var next = $('.readok').length + $('.readerror').length;
        ...
        $(".divMessage_ddlhhu").show();
        $(".divMessage_ddlhhu").html('Đọc THÀNH CÔNG <b>' + $('.readok').length + '</b> điểm đo. </br>'
              + 'Đọc KHÔNG THÀNH CÔNG <b>' + $('.readerror').length + '</b> điểm đo.');
    } catch (e) { console.log(e); }
}
...
function f_load_combo() {
    try {
        var config = {
            connstr: "ConnectOracle_Amiss4",
            namesql: "PKG_DOCDULIEUDCU.GET_ALL_FOLDER",
