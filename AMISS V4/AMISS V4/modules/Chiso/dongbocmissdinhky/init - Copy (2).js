...
function f_thietlapkychot() {
    try {
        ...
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_DONGBOCMIS.ThietLapNgayChot", callback: "f_result_UPDATE_FIX_DAY3" };
        var para = {
            v_fixday: v_fixday,
            v_circle: v_circle,
...
function checkvalidate() {
    ...
    if (p.date_thang == "") return "Vui lòng chọn tháng chốt trước khi thực hiện";
    return "";
}
...
function xuatexcel_cb(iDowdload) {
    try {
        // save data in table AM_TEMP_DBCMIS_BACKUP
        var type = $(iDowdload).closest('div').data('typecb');
        var namef_l = 'dongbocmiss' + $("#dt_thangnapcmis").val().replace("/","_");
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_DONGBOCMIS.EXPORTFILEEXCELCMISS", namefile: namef_l };
        var para = {
            v_IDRANDOM: idramdom_cmis,
            v_CANHBAO: type
        };
...
function fn_export_historyfilexml() {
    ...
    try{
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_DONGBOCMIS.EXPORTHISTORYFILEXML", callback: "f_result_exprotfilenamexml" };
        var para = {
