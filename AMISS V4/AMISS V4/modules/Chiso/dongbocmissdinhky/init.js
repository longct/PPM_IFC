...
$(document).ready(function () {
    ...
    $("#btnTieptuccmis_2").click(function () {
        var timeaddday = convertdate(timeaddday1(ngaychotcmis, 2));
        var timesubday = convertdate(timeaddday1(ngaychotcmis, -2));
        var ngaychon = convertdate($("#dt_thangnapcmis").val());
        var ngaychot= convertdate(ngaychotcmis);
        var t = new Date();
        var mtoday = (t.getMonth() + 1).toString().length == 1 ? "0" + (t.getMonth() + 1) : (t.getMonth() + 1);
        var ytoday = t.getFullYear().toString().length == 1 ? "0" + t.getFullYear() : t.getFullYear();
        if (ngaychot.getFullYear() != ytoday) {
            showToast('Vui lòng chọn lại file cmis ngày mới trong năm ' + ytoday, 'error'); return;
        }
        if (ngaychot.getMonth() != mtoday && ngaychot.getFullYear() == ytoday) {
            showToast('Vui lòng chọn lại file cmis ngày mới trong tháng ' + mtoday + "/" + ytoday, 'error'); return;
        }
        else {
            if (ngaychon > timesubday && ngaychon < timeaddday) {
                step(3);
               // f_thietlapkychot();
            } else {
                showToast('Ngày chốt chỉ số sai lệch không quá +/- 1 ngày', 'error'); return;
            }
        }
    });
...
//        var config = { connstr: "ConnectOracle_Amiss4", namesql: "AMISS_THONGTINDIEMDO.UPDATE_FIX_DAY3", callback: "f_result_UPDATE_FIX_DAY3" };
//        var para = {
//            v_fixday: v_fixday,
//            v_timeday: v_timeday,
//            v_circle: v_circle,
...
function f_napcmiss() {
    try {
        ...
        var v_TimeInput = p.dt_thangnapcmis;
...
function f_result_NapCMIS(config, para, lst) {
    try {
        ...
        $('#saithongtin').html('Có <b>' + sl_saithongtin + '/' + total + '</b> Khách hàng không tồn tại' + (sl_saithongtin == 0 ? "" : i_download));
        $('#slquacao').html('Có <b>' + sl_quacao +  '/' + total + '</b> Khách hàng có SL mới lớn hơn sản lượng cũ ' + para.v_tren + '% ' + (sl_quacao == 0 ? "" : i_download));
        $('#slquathap').html('Có <b>' + sl_quathap + '/' + total + '</b> Khách hàng có SL mới nhỏ hơn sản lượng cũ ' + para.v_duoi + '%' + (sl_quathap == 0 ? "" : i_download));
        $('#khongcochiso').html('Có <b>' + sl_kocochiso + '/' + total + '</b> Khách hàng Không có chỉ số' + (sl_kocochiso == 0 ? "" : i_download));
        $('#slam').html('Có <b>' + sl_am + '/' + total + '</b> Khách hàng Sản lượng âm do hư hỏng hoặc thay công tơ ' + (sl_am == 0 ? "" : i_download));
        $('#slquanho').html('Có <b>' + sl_quanho + '/' + total + '</b> Khách hàng có SL quá nhỏ (<0.5)' + (sl_quanho == 0 ? "" : i_download));
        $('#slkhongtang').html('Có <b>' + sl_khongtang + '/' + total + '</b> Khách hàng có SL không tăng' + (sl_khongtang == 0 ? "" : i_download));
