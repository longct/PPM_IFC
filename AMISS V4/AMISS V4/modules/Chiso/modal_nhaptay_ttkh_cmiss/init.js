...
function checknull() {
    try {
        var p = getAllIdMod();
        if ($('#jqxDropDL_nhapkh').jqxTree('getSelectedItem') == null) return "Vui lòng chọn điện lực";
...
function save_nkhcmiss() {
    try {
        ...
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_NHAPKHTUCMISS.INSERT_DIEMDO", callback: "f_result_insert_diemdo" };
        var para = {
...
function f_GetData_ComboDL_ntkh() {
    try {
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_TREE.GET_ALL_FOLDER", callback: "f_result_GetData_ComboDL_ntkh" };
        var para = {
            v_Code: "01",
            v_TypeTree: "01"
        };
...
function f_loadchot(id) {
    try {
        var config = {
            ...
            namesql: "AMISS_THONGTINDIEMDO.GETINFONODECMIS",
            callback: "f_result_GETINFONODECMIS"
        };
        var para = {
            v_Value: id,
            v_TypeNode: 1
