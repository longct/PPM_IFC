    ...
    try {
        ...
        if (localStorage.getItem("tree_node")) {
            ...
            if (istype != "0" && istype != "4" && socongto != "0")
                $("#mesMain").html(tendiemdo + "(Số công tơ: " + socongto + "- Loại điểm đo: " + replacePha(istype) + ")");
            else if (istype == "4")
                $("#mesMain").html("Sổ ghi: " + tendiemdo);
            else if (socongto == "0" && istype != "4")
                $("#mesMain").html("Đơn vị: " + tendiemdo);
            $("#mesMain").css("color", "#444");
        } else {
        ...
        $("#help_csttd_1pha").click(function () {
            $('#csttd_3pha').modal("show");
            f_trogiup();
...
function result_cscngay_kh(config, para, lst) {
    try {
        ...
        stopLoad();
        LoadPhanTrang_("pageLst_csttd_kh", "pageCurent_csttd_kh", data, function () {
            get_CSCNgay_KH($("#pagenumber").val() - 1);
        });
    } catch (e) {
        console.log(e);
    }
...
function result_cscthang_kh(config, para, lst) {
    try {
        ...
        stopLoad();
        LoadPhanTrang_("pageLst_csttd_kh", "pageCurent_csttd_kh", data, function () {
            get_CSCThang_KH($("#pagenumber").val() - 1);
        });
    } catch (e) {
        console.log(e);
    }
...
function result_cscthang_list_kh(config, para, lst) {
    try {
        ...
        stopLoad();
        LoadPhanTrang_("pageLst_csttd_kh", "pageCurent_csttd_kh", data, function () {
            get_CSCThang_list_KH($("#pagenumber").val() - 1);
        });
    } catch (e) {
        console.log(e);
    }
