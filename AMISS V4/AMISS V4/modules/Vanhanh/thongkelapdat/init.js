...
function get_TKLD(page) {
    try {               
        ...
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THONGKELAPDAT.THONGKELAPDAT", callback: "result_TKLD" };
        var para = {
            v_Code :code , 
            v_TuNgay :tungay , 
            v_DenNgay :denngay , 
...
function result_TKLD(config, para, lst) {
    try {
        console.log(lst);
        if (lst == null || lst == "[]" || lst == undefined) {
            $(".sobanghi").html("Tổng số 0 <span tkey='banghi'></span>");
            return;
        }
        var data = lst.data;
        var tr = "";
        ...
        LoadPhanTrang("pageLst_tkld", "pageCurent_tkld", data, function () {
            var p = getAllIdMod();
...
function excel() {
    ...
    var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THONGKELAPDAT.THONGKELAPDAT", namefile: namef_l };
    var para = {
        v_Code: code,
        v_TuNgay: tungay,
        v_DenNgay: denngay,
    ...
    var colum = {
        ...
        { field: "madiemdo", name: "Mã điểm đo", type: "TextAndBold" },
        { field: "tendiemdo", name: "Tên khách hàng", type: "TextAndBold" },
        { field: "socongto", name: "Số công tơ", type: "TextAndBoldCenter" },
        { field: "trangthaimodem", name: "Trạng thái modem", type: "TextAndBoldCenter" },
