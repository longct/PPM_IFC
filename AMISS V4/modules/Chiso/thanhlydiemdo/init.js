...
$(document).ready(function () {
    try {
        $("#txt_filettldd").change(function () {
            var fname = $("#txt_filettldd").get(0).files[0].name;
        ...
        $("#btnthanhlydd").click(function () {
            var lenght_nkhtcmis = $("#txt_filettldd").get(0).files.length;
...
function f_UploadFile_ttxuatexcel() {
    try {
        idram_khdd = Math.random().toString(36).substr(2);
        var fdata = new FormData();
        var file = $('#txt_filettldd')[0].files[0];
        ...
        fdata.append("insertto", "TEMP_THANHLYDIEMDO");
        fdata.append("idrandom", idram_khdd);
...
function capnhatthanhlydiemdo() {
    try {
        ...
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THANHLY.UPDATETHANHLY", callback: "result_capnhatthanhlydiemdo" };
        var para = {
