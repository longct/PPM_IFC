...
$(document).ready(function () {
    try {
        ...
        selectlang();
        initformelement();
        $('#txt_file_thayct').click(function () {
            step(0);
            $('#txt_file_thayct').val("");
        });
        $("#txt_file_thayct").change(function () {
            step(1);
            if ($('#txt_file_thayct').get(0).files.length > 0) {
...
function step(s) {
    switch (s) {
        case 0:
            $("#btnkiemtra_thaytocmis").hide();
            $("#btnThietlap_thaytocmis").hide();
            $("#step_" + (s + 1)).removeClass("step-active");
            $("#step_" + (s + 1) + " .step_stt div").removeClass("step_done");
            $("#step_" + (s + 1) + " .step_stt div").addClass("step_deactive");

            $("#step_" + (s + 2)).removeClass("step-active");
            $("#step_" + (s + 2) + " .step_stt div").removeClass("step_done");
            $("#step_" + (s + 2) + " .step_stt div").addClass("step_deactive");

            $("#step_" + (s + 3)).removeClass("step-active");
            $("#step_" + (s + 3) + " .step_stt div").removeClass("step_done");
            $("#step_" + (s + 3) + " .step_stt div").addClass("step_deactive");
        case 1:
            console.log(1);
            if ($('#txt_file_thayct').get(0).files.length === 0) {
                $("#btnkiemtra_thaytocmis").hide();
                $("#btnThietlap_thaytocmis").hide();
                $("#step_" + s).removeClass("step-active");
                $("#step_" + s + " .step_stt div").removeClass("step_done");
                $("#step_" + s + " .step_stt div").addClass("step_deactive");
            }
            else {
                $("#btnkiemtra_thaytocmis").show();
                //$("#btnThietlap_thaytocmis").show();
...
function f_thaycongtocmis() {
    try {
        ...
        fdata.append("insertto", "TEMP_DBCMIS_NEW");
...
function Xoa_thaycmiss() {
    try {
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THAYCONGTOCMISS.XOABANG", callback: "result_xoa" };
        var para = {
            v_ID: 'KT'
        };
...
function loaddulieuthaycongto() {
    try {
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THAYCONGTOCMISS.LOADDANHSACH", callback: "result_loaddulieuthaycongto" };
        var para = {
            v_ID: 'KT'
        };
        ExecuteServiceSyns(config, para);
...
function capnhatthaycmiss() {
    try {
        ...
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THAYCONGTOCMISS.TREOTHAO_THUCHIEN", callback: "result_capnhatthaycmiss" };
...
function kiemtra_thaycmiss() {
    try {
        ...
        sctkbao = 0;
        sctkontai = 0;
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_THAYCONGTOCMISS.TREOTHAO_KIEMTRA", callback: "result_kiemtra_thaycmiss" };
        $.each(datacmiss, function (key, val) {
            var para = {
    ...
    try {
        ...
        if (datacmiss.length == count) {
            ...
            $(".messinfo_thaycmiss").html(" <b>" + capnhat + "</b> bản ghi cập nhập chỉ số tháo </br>"
                                           + "<b>" + tontai + "</b> khách hàng có trên hệ thống<br/>"
                                           + "<b>" + sctkontai + "</b> khách hàng Không có trên hệ thống<br/>"
                                           + "<b>" + sctkbao + "</b> khách hàng có số công tơ nằm trong chờ khai báo ");
        }
