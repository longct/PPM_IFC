$(document).ready(function () {
    ...
    $("#chondonvi").change(function () {
        if ($("#chondonvi option:selected").val() == "-1") {
            ...
            //console.log($("#chondonvi option:selected").val());
        } else {
            loadTramToCombo($("#chondonvi option:selected").val());
            localStorage.setItem("CT", $("#chondonvi option:selected").val());
...
function getTreeForUser(usercode) {
    try {
        ...
        var config = { namesql: dec("QU1JU1NfQ0FZRFVMSUVVLg==") + dec("R2V0VHJlZUFsbA=="), callback: "f_result_getTreeForUser", connstr: "ConnectOracle_Amiss4" };
        var para = {
...
function loadProvincetoCombo(code) {
    try {
        ...
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_TREE.GetProvinceForCombo", callback: "result_loadProvince" };
        var para = {
            v_Code: code,
...
function result_loadProvince(config, para, lst) {
    ...
    $.each(data, function (key, val) {
        if (val.code == localStorage.getItem("CT")) {
            $('#chondonvi').append($('<option>', {
                value: val.code,
                text: val.name,
                selected: true
...
function loadTramToCombo(code) {
    try {
        //console.log(code);
        var v_UserCode = JSON.parse(localStorage.getItem("userinfo")).code;
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_TREE.GetProvinceForCombo", callback: "result_loadTram" };
        var para = {
            v_Code: code,
            v_UserCode: v_UserCode,
            v_isSub: 0
...
function result_loadTram(config, para, lst) {
  ...
  //  console.log(data);
    ...
    $.each(data, function (key, val) {
        if (val.code == localStorage.getItem("DL")) {
            //console.log(val);
            $('#chontram').append($('<option>', {
                value: val.code,
                text: val.name,
