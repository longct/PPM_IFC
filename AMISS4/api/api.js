        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (config.callback != null && config.callback != '') {
                    ...
                    console.log("4444");
                    eval(config.callback)(config, para, lstData);
                }
            },
            complete: function () {
            },
...
function getModules(uspe) {
    var mod = [];
    $.each(uspe, function (index, value) {
        //console.log(uspe);
        mod.push(value.substring(6, 9));
    });
...
//  lấy ngày hiện tại lùi 1 tuần 30/06/2016
...
//  lấy ngày hiện tại lùi 8 ngay 2016-06-01
function gettimeweek() {
    try {
        var myDate = new Date();
        var t = new Date(myDate.getTime() - (60 * 60 * 24 * 8 * 1000));
        var d = t.getDate().toString().length == 1 ? "0" + t.getDate() : t.getDate();
        var m = (t.getMonth() + 1).toString().length == 1 ? "0" + (t.getMonth() + 1) : (t.getMonth() + 1);
        var y = t.getFullYear().toString().length == 1 ? "0" + t.getFullYear() : t.getFullYear();
        var tt = y + "-" + m + "-" + d;
...
function f_callToServiceAsmx(config, para)
{
    var lstData;
    var parass = JSON.stringify(para);
    $.ajax({
        type: "POST",
        url: config.url,
        data:parass, // the data in JSON format.  Note it is *not* a JSON object, is is a literal string in JSON format
...
function getValueXml(xml, name) {
    try {
        return xml.find(name).text();
    } catch (e) { return ""; }
}
function getValueXmlnumbertxt(xml, name) {
    try {
        return $(xml).find(name).text();
    } catch (e) { return 0; }
}
function getValueXmltxt(xml, name) {
    try {
        return $(xml).find(name).text();
    } catch (e) { return ""; }
}
