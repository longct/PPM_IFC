...
function get_Thongkeloi_chitiet() {
    try {
        var code = JSON.parse(localStorage.getItem("tree_node"))[0].id;
        var config = { connstr: "ConnectOracle233", namesql: "AMISS_LOIDIEMDO.THONGKELOICHITIET", callback: "result_thongkelois_chitiet" };
        var para = {
...
function result_thongkelois_chitiet(config, para, lst) {
    try {
        console.log(lst);
        var data = lst.data;
        GData = data;
...
function excel_chitiettrangthai(bang) {
    try {
        ...
        var config = { connstr: "ConnectOracle233", namesql: "AMISS_LOIDIEMDO.THONGKELOICHITIET", namefile: namef_l, exporttable: bang };
        var code = JSON.parse(localStorage.getItem("tree_node"))[0].id; 
        var para = {
            v_Type: 1,// 1: Dienluc(Code); 2: Nhom(UserId); 3:Nhom(GroupId); 4:Tram(CodeDM)
            v_Value: code,
