...
function get_CBVH_MatAp() {
    try {
        ...
        var config = { connstr: "ConnectOracle233", namesql: "AMISS_CANHBAOVANHANH.ThongKeCanhBao_Matdienap", callback: "drawTable_thongkecbvh_matap" };
        var para = {
    ...
    try{
        ...
        $.each(data_, function (key, val) {
            tr += '<tr class="a_c">' +
                        ...
                        '<td class="a_c">' + val.thu2 + '</td>' +
                        '<td class="a_c">' + val.thu3 + '</td>' +
                        '<td class="a_c">' + val.thu4 + '</td>' +
                        '<td class="a_c">' + val.thu5 + '</td>' +
                        '<td class="a_c">' + val.thu6 + '</td>' +
                        '<td class="a_c">' + val.thu7 + '</td>' +
                        '<td class="a_c">' + val.chunhat + '</td>' +
                    '</tr> ';
        });
        $("#thongke_cbvh_matap_details tbody").empty();
...
function excel_locaptheotuan() {
    try {
        ...
        var config = { connstr: "ConnectOracle233", namesql: "AMISS_CANHBAOVANHANH.ThongKeCanhBao_Matdienap", namefile: namef_l };
        var para = {
            v_Type: 1,//-- 1: Dienluc(Code); 2: Nhom(UserId); 3:Nhom(GroupId); 4:Tram(CodeDM)
            v_Value: code,
            v_From: tungay,
        ...
        var colum = {
            ...
            { field: "thu2", name: "Thứ 2", type: "Text" },
            { field: "thu3", name: "Thứ 3", type: "Text" },
            { field: "thu4", name: "Thứ 4", type: "Text" },
            { field: "thu5", name: "Thứ 5", type: "Text" },
            { field: "thu6", name: "Thứ 6", type: "Text" },
            { field: "thu7", name: "Thứ 7", type: "Text" },
            { field: "chunhat", name: "Chủ nhật", type: "Text" }
            ]
        };
