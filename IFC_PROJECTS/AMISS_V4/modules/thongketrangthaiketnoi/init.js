...
function get_Thongkeloi() {
    try {
        ...
        var config = { connstr: "ConnectOracle233", namesql: "AMISS_LOIDIEMDO.THONGKELOI", callback: "draw_bdthongke" };
        var para = {
...
function xuatexecl_thongkettt() {
    try {
        ...
        var code = JSON.parse(localStorage.getItem("tree_node"))[0].id;
        var config = { connstr: "ConnectOracle233", namesql: "AMISS_LOIDIEMDO.THONGKELOI", namefile: namef_l };
        var para = {
            v_Type: 1,// 1: Dienluc(Code); 2: Nhom(UserId); 3:Nhom(GroupId); 4:Tram(CodeDM)
            v_Value: code,
            v_From: '',
