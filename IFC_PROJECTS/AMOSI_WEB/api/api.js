var urli = "http://14.177.66.131:55555/";
    ...
    try {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (config.namesql == "PKG_BAOCAO_HANGNGAY.GetComboDonvi") {
                    sessionStorage.setItem("combodonvi", JSON.stringify(lstData))
                }
            },
            complete: function () {
            },
...
function hideDenTrangTri() {
    var type = window.location.hash.substr(1);
    if (type == "Module/HomeDenTrangTri") {
        $(".chuyenloai").hide();
    }
}
...
function api_uploadFirm_ware(config, fd) {
    try {
        var lstData = null;
        $.ajax({
            type: 'POST',
            url: urli + '/home/MCF_Upload_Firmware',
            data: fd,
            processData: false,
            contentType: false,
            dataType: "json",
