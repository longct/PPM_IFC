...
//Sql1Pha3Pha
var urli = "http://101.99.51.73:8888/ServiceVTPhu";
//var urli = "http://localhost:22660";
//var urli = "http://101.99.51.73:8888/ServiceEMS_NEW";
function ExecuteServiceSyns(config, para) {
    try {
       // config.connstr = "ConnectEMS_DEV"
...
function f_importExcelMaxLength(config,para, fd) {
    try {
        var lstData = null;
        $.ajax({
            type: 'POST',
            url: urli + '/home/ReadExcelMaxLength',
            data: fd,
            processData: false,
            contentType: false,
            dataType: "json",
            success: function (data) {
                lstData = data == null || data == "" ? "[]" : data;
                if (config.callback != null && config.callback != '') {
                    funExeItem(config.callback, config, para, lstData);
                }
            }
