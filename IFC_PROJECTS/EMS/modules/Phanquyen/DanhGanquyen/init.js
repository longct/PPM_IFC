...
function lsttaikhoan() {
    try {
        ...
        var config = { connstr: "ConnectEMS", namesql: "LSTNGUOIDUNG", callback: "result_lsttaikhoan" };
        var para = {
...
function result_lsttaikhoan(config, para, lst) {
    try {
        ...
        dataToCob("cb_taikhoan_apquyen", data, "userid", "ten", "-1", "Chọn tài khoản");
    } catch (e) {
        console.log(e);
    }
}
function lsttaimau() {
    try {
        var userinfo = JSON.parse(localStorage.getItem("userinfo"));
        var config = { connstr: "ConnectEMS", namesql: "LSTMAU", callback: "result_lsttaimau" };
        var para = [];
        ExecuteServiceSyns(config, para);
    } catch (e) {
        console.log(e);
...
function loadmauquyen_apquyen(id) {
    try {
        var config = { connstr: "ConnectEMS", namesql: "IDCHUNGQUYEN", callback: "resut_loadmauquyen_apquyen" };
        var para = {
            v_IDCHUNG: id
        };
        ExecuteServiceSyns(config, para);
...
function loadquyentheouser(id) {
    try {
        var config = { connstr: "ConnectEMS", namesql: "LOADPHANQUYENTAIKHOAN", callback: "resut_loadmauquyen_apquyen" };
        var para = {
            v_USERID: id
        };
        ExecuteServiceSyns(config, para);
...
function layid_ganquyen() {
    try {
        ...
        var dt = '{ "table": ' + JSON.stringify(lstId) + ' }';

        var config = { connstr: "ConnectEMS", namesql: "CAPQUYENUSER", callback: "resut_capnhatquyen_taikhoan" };
        var para = {
...
function clear_taikhoan() {
    try {
        ...
        $("#cb_danhsachmau_apquyen").val("-1");
        $('#cb_taikhoan_apquyen').select2("val", "-1");
