...
$(document).ready(function () {
    try {
        ...
        if (user.code.length > 4) {
            $(".classtrangthai_serial").html("Không có quyền xem trang này");
            return;
        } else {
            loadConetent();
            f_loadBanDau_cntttb();
...
function f_loadBanDau_cntttb() {
    try {
        var userInfo = JSON.parse(localStorage.getItem("userinfo"));
        var config = { namesql: "TB_Export_LstLoadByCode", callback: "f_result_loadBanDau_cntttb", connstr: "ConnectEMS" };
...
function f_result_loadBanDau_cntttb(config, para, lst) {
    try {
        dataToCob("cbtrangthai_cntttb", lst.data, "code", "name", "-1", "--Chọn trạng thái--");
    } catch (e) {
        console.log(e);
    }
}
