...
function f_checkPermisstion() {
    try {
        var permiss = JSON.parse(localStorage.getItem("PermisstionModul"));
       
        $.each(permiss, function (key, val) {
            try {
                if (!val.alowview) {
                    console.log(val.moduleid)
                    $("#" + val.moduleid).empty();
                    $("#" + val.moduleid).append("<div id='delete" + val.moduleid + "'></div>")
                    messInfo("delete" + val.moduleid, "Bạn không có quyền xem trang này", "error");
                }
            } catch (e) {
                console.log(e);
            }
