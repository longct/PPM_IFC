...
function checkkytunull_tinhapltb() {
    try{
        var p = getAllIdMod();
        if (p.txtloaithietbi_tinhapltb == '') return "Không được để loại thiết bị trống";
        return "";
    } catch (e) {
        console.log(e);
    }
    ...
    try {
        ...
        $.each(data[0].kq0, function (key, val) {
            ...
            row += "<tr><td>"
                  + (key+1)+ "</td><td>"
                  + val.name + "</td><td>"
                  + "<form class='form-inline' role=''form'><div class='form-group'><a class='btn btn-success btn-action' data-toggle='modal' href='#sua_nhaploaittbi' style='width: 65px;height: 30px;' id='btn_nhaploaittb" + val.code + "'>Sửa</a></div></form></td> </tr>";
