...
function loadmauquyen_suathem(id) {
    try {
        clear_load_sua();
        idchungquyen = id;
        var config = { connstr: "Oracle_HDDT", namesql: "HD_PHANQUYEN.IDCHUNGQUYEN", callback: "resut_loadmauquyen_suathem" };
        var para = {
            v_IDCHUNG : id
        };
...
function capnhatquyen_sua() {
    try{
        var userinfo = JSON.parse(localStorage.getItem("userinfo"));
        var config = { connstr: "Oracle_HDDT", namesql: "HD_PHANQUYEN.SUAQUYENMOI", callback: "resut_capnhatquyen_sua" };
        var para = {
