...
function f_GetData_ComboDL() {
    try {
        var config = { connstr: "ConnectOracle_Amiss4", namesql: "PKG_TREE.GET_ALL_FOLDER", callback: "f_result_GetParentNodeAllTree" };
        var para = {
            v_Code: "01",
            v_TypeTree: "01"
        };
...
function f_result_getdatakhcmiss(config, para, lst) {
    try {
        ...
        $(".divMessage_cmiskh").html('<b>' + countmdd_sct1 + '</b> bản ghi CÓ mã điểm đo và CÓ số công tơ. => Cập nhật thông tin điểm đo </br>'
              ...
              + '<b>' + countmdd_sct5 + '</b> bản ghi CÓ mã điểm đo và không có số công tơ => Tự động khai báo mới điểm đo</br>'
