        ...
        $.each(lstBong, function (i, marker) {
            if (marker.latitude != null && marker.longitude != null) {
                ...
                $('#map_canvas_tqtbds').gmap('addMarker', {
                    ...
                    'icon': marker.status == "0" ? 'img/gmap/trangthai0.png' :
                        marker.status == "1" ? 'img/gmap/trangthai1.png' :
                        marker.status == "2" ? 'img/gmap/trangthai2.png' :
                        'img/gmap/trangthai.png',
                }).click(function () {
...
function clickChangeTap(meterid) {
    try {
        var chitiettu = { id: meterid, userinfo: JSON.parse(localStorage.getItem("userinfo")) };
        localStorage.setItem("chitiettu", JSON.stringify(chitiettu));
        $(location).attr('href', 'master.html#Module/chitietbong');

        //var chitiettu = { id: page, userinfo: JSON.parse(localStorage.getItem("userinfo")) };
        //localStorage.setItem("chitiettu", JSON.stringify(chitiettu));
        //$(location).attr('href', 'master.html#Module/chitietbong');
      //  window.open('master.html#Module/chitietbong', '_blank');
    } catch (e) {
        console.log(e);
    }
}
...
function draw_Map(list) {
    ...
    try {
        ...
        $('#map_canvas_tqtbds').gmap('option', 'zoom', 20);
...
function khoitaosukien_bds(id) {
    ...
    $(".chitiet1bong_bando").click(function () {
        var chitiettu = { id: id, userinfo: JSON.parse(localStorage.getItem("userinfo")) };
        localStorage.setItem("chitiettu", JSON.stringify(chitiettu));
