$(document).ready(function () {
    try {
        //loadConetent();
        $('#menuThietlap_canhbao').click(function () {
            loadchecklog_master();
            loadcbthietlap();
            var lstTu = JSON.parse(localStorage.getItem("tuSelected"));
            //if (tree_node[0].tu.length != 1 || tree_node[0].quan.length != 0) {
            //    $('#thietlap_canhbao_content').hide();
            //    messInfo("messinfo_tlcb", 'Vui lòng 1 tủ nhất định để thiết lập cảnh báo', "error");
            //} else {
            //    $('#thietlap_canhbao_content').show();
            //    $('#messinfo_tlcb').hide();
            //}
        });
        ...
        $("#btn_apdungC_tlcb").click(function () {
            ...
            if (val == "-1") {
                ...
                var tree_node = JSON.parse(localStorage.getItem("tree_node"));
...
function loadthongtinIdthietlap(val) {
    try {
        var config = { namesql: "PKG_THIETLAPCANHBAO.LOAD_THIETLAP_CB", callback: "f_result_loadthongtinIdthietlap", connstr: "ConnectOracleStreetLight" };
        var para = {
            v_ID: val
        };
        ExecuteServiceSyns(config, para, false);
    ...
    try {
        if (localStorage.getItem("tuSelected")) {
            ...
            if (localStorage.getItem("datat_b")) {
                ...
                messInfo("messinfo_tlcb", 'Vui lòng chọn tủ hoặc quận để xem cảnh báo', "error");
                return;
            }
        }
        else {
        ...
        var userinfo = JSON.parse(localStorage.getItem('userinfo'));
        var config = { namesql: "PKG_THIETLAPCANHBAO.APDUNG_CANHBAO", callback: "f_result_apdungcauhinh", connstr: "ConnectOracleStreetLight" };
        var para = {
            v_userid: userinfo.idnhanvien,
            v_ID_THIETLAP: id,
            v_meterid: lstId
