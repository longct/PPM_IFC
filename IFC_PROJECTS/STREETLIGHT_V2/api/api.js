...
var urli = "http://14.177.66.131:5566/ServiceSqlOracleNew"
//var urli = "http://streetlights.ifc.com.vn/SERVICESTREETLIGHT";

var urliTcp = "https://tcpip.amiss.com.vn/ServiceTCPAll";
...
function checkAutoRefresh() {
    ...
    if (localStorage.getItem("map") == "auto") {
        loadBong = setInterval(function () {
            if (localStorage.getItem("tuSelected") && JSON.parse(localStorage.getItem("tuSelected")).length == 1) {
...
function api_ExecuteTcpString(config, para) {
    try {
        ...
        $.ajax({
            url: urliTcp + "/api/TCP_OverServiceTcpString",
            type: "POST",
            data: jsonpara,
            async: syns,
            success: function (data, textStatus, jqXHR) {
...
function api_ExecuteTcpByte(config, para) {
    try {
        ...
        $.ajax({
            url: urliTcp + "/api/TCP_OverServiceTcpByte",
            type: "POST",
            data: jsonpara,
            async: syns,
            success: function (data, textStatus, jqXHR) {
