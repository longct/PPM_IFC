...
$(document).ready(function () {
    try {
        ...
        loadchecklog_master();
        init_Dktdtd();
        $('#tudong_thietlap').change(function () {
            if ($('#tudong_thietlap').is(":checked")) {
                $('#addHtml_Dktdtd').hide();
                $('#khung_nhapgio').show();
            }
            else {
        ...
        $('#btn_clickde').click(function () {
            if ($('#txt_nhapgiode').val() == "" || parseInt($('#txt_nhapgiode').val()) > 24 || parseInt($('#txt_nhapgiode').val()) < 0) {
                alert("Chưa nhập giờ hoặc sai định dạng giờ");
            }
            else {
                var gio_ = $('#txt_nhapgiode').val();
                var phut_ = $('#txt_nhapphutde').val();
                genCauhinh(gio_,phut_);
            }
        })
...
function genCauhinh(gio,phut) {
    ...
    var phut_2 = parseInt(phut_) + 1;
    var phut_3;
    var phut_4;

    var str = '<tbody><tr style="line-height: 18px;">' +
        '<td style="padding-left:21px;">Từ</td>' +
        '<td style="padding-left:18px;">Đến</td>' +
        '<td style="text-align:left;"><div class="phatxt"><span>Chế độ</span></div></td>  </tr>' +
        '<tr id="row0" style="overflow:auto;"><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="FromHour_Dktdtd0" value="' + gio_ + ':' + (parseInt(phut_) + 1 )+ '" style="width: 60px;" readonly="readonly"></td><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="ToHour_Dktdtd0" value="' + gio_ + ':' + (parseInt(phut_) + 2) + '" style="width: 60px;" readonly="readonly"></td><td style="width: 60%; vertical-align: middle;"><div style="padding: 0 15px; margin-top: 0px">' +
        '<select class="form-control input-sm m-bot15" id="Brightness_Dktdtd0">' +
        '<option value="100">100%</option>' +
        '</select><div></div></div></td></tr>' +
        '<tr id="row1" style="overflow:auto;">' +
        '<td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="FromHour_Dktdtd1" value="' + gio_ + ':' + (parseInt(phut_) + 2) + '" style="width: 60px;" readonly="readonly"></td><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="ToHour_Dktdtd1" value="' + gio_ + ':' + (parseInt(phut_) + 3) + '" style="width: 60px;" readonly="readonly"></td><td style="width: 60%; vertical-align: middle;"><div style="padding: 0 15px; margin-top: 0px">' +
        '<select class="form-control input-sm m-bot15" id="Brightness_Dktdtd1">' +
        '<option value="30">30%</option>' +
        '</select><div></div></div></td></tr>' +
        '<tr id="row2" style="overflow:auto;"><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="FromHour_Dktdtd2" value="' + gio_ + ':' + (parseInt(phut_) +3) + '" style="width: 60px;" readonly="readonly"></td><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="ToHour_Dktdtd2" value="' + gio_ + ':' + (parseInt(phut_) + 4) + '" style="width: 60px;" readonly="readonly"></td><td style="width: 60%; vertical-align: middle;"><div style="padding: 0 15px; margin-top: 0px">' +
        '<select class="form-control input-sm m-bot15" id="Brightness_Dktdtd2">' +
        '<option value="100">100%</option>' +
        '</select><div></div></div></td></tr>' +
        '<tr id="row2" style="overflow:auto;"><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="FromHour_Dktdtd3" value="' + gio_ + ':' + (parseInt(phut_) + 4) + '" style="width: 60px;" readonly="readonly"></td><td style="width: 20%" class="inp_txt">' +
        '<input type="text" id="ToHour_Dktdtd3" value="24:00" style="width: 60px;" readonly="readonly"></td><td style="width: 60%; vertical-align: middle;"><div style="padding: 0 15px; margin-top: 0px">' +
        '<select class="form-control input-sm m-bot15" id="Brightness_Dktdtd2">' +
        '<option value="0">Off</option>' +
        '</select><div></div></div></td></tr></tbody>';

    $('#addHtml_Dktdtd').empty();
    $('#addHtml_Dktdtd').append(str);
    $('#addHtml_Dktdtd').show();
}
function genGioPhut(h, m, index) {
    if (h < 10) h = "0" + h;
    if (m < 10) h = "0" + m;
    if (index == 1) {
        return h + ":" + m;
