...
function insertTempTable_Dkdtt() {
    try {
        alert(2);
        var strW = ""
        if ($("#phaA").hasClass("phaOff") || $("#phaB").hasClass("phaOff") || $("#phaC").hasClass("phaOff") | $("#md").val() == "Tủ đang mất điện") {
            //alert("Lệnh chỉ thực hiện với các bóng thuộc Nhánh đang bật (ON)");
            $("#thongbao_nhanhoff").modal("show");
            //return;
        }
...
function checkTxt_Dkdtt() {
    try {
        ...
        if ($("#Brightness_Dkdtt").val() == "-1")
            return "Vui lòng chọn chế độ sáng";
        return "";
