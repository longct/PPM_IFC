        ...
        $('#btnThem_Dkttd').click(function() {
            ...
            if (slider_Dkttd.result.to < valTimeSl_Dkttd.length-1) {
                ...
                slider_Dkttd.update({
                    ...
                    to: (slider_Dkttd.result.to + (parseInt($("#khoang_Dkttd").val()) >0? parseInt($("#khoang_Dkttd").val()) :120)),
                    disable: false
                });
...
function insertTempTable_Dkttd()
{
    try
    {
        var rowCount = $('#addHtml_Dkttd >tbody >tr').length;
...
function init_Dkttd()
{
    try
    {
       ...
       var slider_Dkttd = $("#TimeSl_Dkttd").data("ionRangeSlider");
        ...
        $('#addHtml_Dkttd').append(addHtmlHeader_Dkttd());
        $('#addHtml_Dkttd').append(addHtml_Dkttd("00:00", time,"0","0","0", 0));
...
function addHtmlHeader_Dkttd()
{
    try {
        ...
        return "<tr style='line-height: 18px;'> <td style='padding-left:21px;'>Từ</td>  <td style='padding-left:18px;'>Đến</td>   <td style='text-align:left;'><div class='phatxt'><span>Nhánh 1</span><span class='" + (lstId_Dkttd[0].loaithietbi == null || lstId_Dkttd[0].loaithietbi == undefined || lstId_Dkttd[0].loaithietbi == "tu" ? "" : "chuyenloai") + "'>Nhánh 2</span><span  class='" + (lstId_Dkttd[0].loaithietbi == "tu" ? "" : "chuyenloai") + "'>Nhánh 3</span></div></td>  </tr> ";
    } catch (e) {
        console.log(e);
    }
}
...
function loadtu_cauhinhpha() {
    try {
        var config = { namesql: "PKG_CSTHIEPLAP.LSTCLFILE", callback: "f_result_loadtu_cauhinhpha", connstr: "ConnectOracleStreetLight" };
        var para = { v_TYPE: 'PHA' };
...
function f_result_loadtu_cauhinhpha(config, para, lst) {
    try {
        var data = lst.data;
        if (data == '' || data.length == 0 || data == '[]' || data == undefined) {
            return;
        }
        dataToCob("cb_phatudong", data, "id", "ten", "-1", "Chọn cấu hình có sẵn");
    } catch (e) {
        console.log(e);
    }
}

function f_loadInfoOne() {
    try {
        messInfo("messinfo_Dkttd", "", "error");
        var config = {
            connstr: "ConnectOracleStreetLight",
            namesql: "PKG_CSTHIEPLAP.LOADTHONGTIN",
            callback: "f_result_f_loadInfoOne"
        }
        var p = getAllIdMod();
        var para = { v_ID: p.cb_phatudong }
        ExecuteServiceSyns(config, para);
    } catch (e) { console.log(e); }
}

function f_result_f_loadInfoOne(config, para, lst) {
    if (lst == null || lst == "" || lst == "[]" || lst.data.length == 0) {
        init_Dkttd();
        return;
    }
    ...
    $.each(data, function (key, val) {
        ...
        $('#btnThem_Dkttd').attr('disabled', 'disabled');
     // $('#btn_CapNhat_Dkttd').attr('disabled', 'disabled');
        onOff_Dkttd(key);
...
function checkTxtLenh_Dkttd() {
    try {
        var p = getAllIdMod();
        if (p.khoang_Dkttd.length > 0) {
            if ($.isNumeric(p.khoang_Dkttd) == false) return "Số phút nhập không được bỏ trống";
            if (p.khoang_Dkttd > 1440) return "Số phút nhập lớn hơn 1440 phút";
        }
