...
//var urli = "http://14.177.66.131:5566/ServiceSqlOracleNew"
var urli = "http://192.168.1.246/ServiceChung";

var urliTcp = "http://192.168.1.246/ServiceChung";
//var urli = "http://streetlight.ifc.com.vn/Service";

//var urliTcp = "http://streetlight.ifc.com.vn/Service";
//var urliTcp = "http://localhost/ServiceTcp";
//var KetNoiToiOracle = "ConnectOracle_BacNinh";
//var KetNoiToiOracle = "ConnectOracle_NamDinh";
//var KetNoiToiOracle = "ConnectOracle_CaoBang";
//var KetNoiToiOracle = "ConnectOracle_HoaBinh";
//var KetNoiToiOracle = "ConnectOracle_PhuTho";
//var KetNoiToiOracle = "ConnectOracle_HaNam";
//var KetNoiToiOracle = "ConnectOracle_NgheAn";
//var KetNoiToiOracle = "ConnectOracle_SonLa";
//var KetNoiToiOracle = "ConnectOracle_BacGiang";
//var KetNoiToiOracle = "ConnectOracle_DienBien";
//var KetNoiToiOracle = "ConnectOracle_BacKan";
//var KetNoiToiOracle = "ConnectOracle_LangSon";
//var KetNoiToiOracle = "ConnectOracle_ThaiNguyen";
//var KetNoiToiOracle = "ConnectOracle_ThanhHoa";
//var KetNoiToiOracle = "ConnectOracle_233";
var load = true;
$(document).keydown(function (e) {
    if (e.keyCode == 16) {
        console.log("đang điều khiển");
        load = false;
    }
    else {
        load = true;
...
setInterval(function () {
    var slt = $("#using_json_2").jstree("get_selected")

    if ($(".modal-backdrop.fade").hasClass("in") || slt.length > 1 || load == false) {
        console.log("đang điều khiển");
    } else {
        $('#cb_donvi').change();
    }

    //getTree($('#cb_donvi option:selected').val(), 'root');
}, 5000);
function resize() {
...
// set hai ngay voi nhau
var compareDates01 = function (from, to) {
    try {
        var startTime = new Date(from);
        var endTime = new Date(to);
        var dateResult = endTime.getTime() - startTime.getTime(); // This will give difference in milliseconds
...
function f_getNextTime() {
    try {
        ...
        for (var i = 0; i < 1080; i++) {
            ...
            var t1 = t.setMinutes(t.getMinutes() + (i * 1));
...
function checkAutoRefresh() {
    if (localStorage.getItem("cb") == "auto") {
        cb_auto = setInterval(function () {
    ...
    //////////////////////////////////////////////
    //if (localStorage.getItem("log") == "auto") {
    //    log_auto = setInterval(function () {
    ...
    //}
    //////////////////////////////////////////////
    if (localStorage.getItem("map") == "auto") {
        loadBong = setInterval(function () {
            //console.log($(".jstree-clicked"));
            if (JSON.parse(localStorage.getItem("quan")).length == 0) {
                //console.log($("#using_json_2 li[id=" + JSON.parse(localStorage.getItem("tuSelected"))[0] + "] a"));
                ...
                $("#" + JSON.parse(localStorage.getItem("quan"))[0] + "_anchor").click()
                //console.log("#" + JSON.parse(localStorage.getItem("quan"))[0] + "_anchor");
...
function api_ExecuteTcpString(config, para) {
    try {
        ...
        $.ajax({
            url: urliTcp + "/api/TCP_SendByteStreetlight",
            type: "POST",
            data: jsonpara,
            async: syns,
            success: function (data, textStatus, jqXHR) {
...
function api_ExecuteTcpByte_New(config, para, type) {
    try {

        var jsonpara = { config: JSON.stringify(config), para: JSON.stringify(para), type: type };
        console.log(jsonpara);
        var syns = false;
            ...
            success: function (data, textStatus, jqXHR) {
                try {
                    ...
                    if (config.callback != null) {
                        funExeItem(config.callback, config, para, JSON.parse(data));
                    }
                } catch (e) {
                    console.log(e);
                }
