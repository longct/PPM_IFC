...
function loadthoitruc() {
    try{
        var config = { namesql: "THONGTINTRAM.THOIGIANTRUC", callback: "f_result_loadthoitruc", connstr: "ConnOracleXangDau" };
        var para = [''];
...
function loaddanhsachcatruc() {
    try{
        var config = { namesql: "THONGTINTRAM.LOADCATRUC", callback: "f_result_loaddanhsachcatruc", connstr: "ConnOracleXangDau" };
...
function capnhapcatruc() {
    try {
        var p = getAllIdMod();
        var config = { namesql: "THONGTINTRAM.CAPNHATCATRUC", callback: "f_result_capnhatcatrucdata", connstr: "ConnOracleXangDau" };
        var infotree = JSON.parse(localStorage.getItem('infotree'));
        var para = {
            v_maid: '',
...
function chekckdieukien() {
    try{
        ...
        if (datasetbd > datasetkt) {
            return "Thời gian bắt đầu phải nhỏ hơn thời gian kết thúc";
        }
        if (datasetbd == datasetkt) {
            return "Thời gian bắt đầu không được bằng thời gian kết thúc";
        }
...
function load_xoacatruc(val) {
    try{
        var p = getAllIdMod();
        var config = { namesql: "THONGTINTRAM.CAPNHATCATRUC", callback: "f_result_xoacatrucdata", connstr: "ConnOracleXangDau" };
        var infotree = JSON.parse(localStorage.getItem('infotree'));
        var para = {
            v_maid:val,
            v_matram: '',
