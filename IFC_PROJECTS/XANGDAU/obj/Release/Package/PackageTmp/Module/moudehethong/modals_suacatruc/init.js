...
function loadthoi_suatruc() {
    try {
        var config = { namesql: "THONGTINTRAM.THOIGIANTRUC", callback: "f_result_loadthoi_suatruc", connstr: "ConnOracleXangDau" };
        var para = [''];
        ExecuteServiceSyns(config, para);
    } catch (e) {
        console.log(e);
...
function loadthongtin_suaca(val) {
    try {
        ...
        var config = { namesql: "THONGTINTRAM.GETIDCATRUC", callback: "f_result_loadthongtin_suacatruc", connstr: "ConnOracleXangDau" };
        var para = {
            v_maid:val
        };
        ExecuteServiceSyns(config, para);
...
function load_capnhapthongtinsuacatruc() {
    try {
       ...
       var idma= localStorage.getItem("txt_sua");
        var config = { namesql: "THONGTINTRAM.CAPNHATCATRUC", callback: "f_result_capnhapthongtinsuacatruc", connstr: "ConnOracleXangDau" };
        var para = {
            v_maid: idma,
            v_matram: '',
            v_catruc: '',
